...
def classify_time_period(hour_col, day_col):
    ...
    # Work time: 10:00 AM to 7:00 PM (Monday to Saturday)
    work_time = (total_minutes >= 600) & (total_minutes < 1140) & (day_col != 1)

    # Home time: 7:30 PM to 9:30 AM
    home_time = (total_minutes >= 1170) | (total_minutes < 570)

    # Weekend activity time (Marine Lines): 7:00 PM to 11:00 PM on Sunday
    weekend_activity = (day_col == 1) & (total_minutes >= 1140) & (total_minutes < 1380)

    return when(work_time, "Work Time") \
        .when(home_time, "Home Time") \
        .when(weekend_activity, "Weekend Activity") \
...
# Aggregate to get the most frequent home, work, and weekend activity locations
...
weekend_activity_location = home_work_locations.filter(col("time_period") == "Weekend Activity") \
    .groupBy("stay_latitude", "stay_longitude") \
    .agg(count("*").alias("count")).orderBy(desc("count")).first()
...
print("Weekend Activity Location (Marine Lines):")
if weekend_activity_location:
    print(
        f"Latitude: {weekend_activity_location['stay_latitude']}, Longitude: {weekend_activity_location['stay_longitude']}")
else:
    print("No weekend activity location identified.")
...
add_significant_location(weekend_activity_location, 'Weekend Activity Location')
...
# Add Weekend activity location
if 'weekend_activity_location' in locals() and weekend_activity_location is not None:
    folium.Marker(
        location=[weekend_activity_location['stay_latitude'], weekend_activity_location['stay_longitude']],
        icon=folium.Icon(color='green', icon='leaf'),
        popup=folium.Popup(f"Weekend Activity<br>Latitude: {weekend_activity_location['stay_latitude']:.6f}<br>Longitude: {weekend_activity_location['stay_longitude']:.6f}", max_width=300)
    ).add_to(m)

# Save the map
map_path = "clustered_locations_visualization_8.html"
m.save(map_path)

# Open the map in the default web browser
webbrowser.open('file://' + os.path.realpath(map_path))

print(f"Map has been saved to {os.path.realpath(map_path)}")
