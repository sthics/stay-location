...
df = spark.read.csv("C:/Users/User/Downloads/6.csv", header=True, inferSchema=True)
...
# Filter margin error > 500
df = df.filter(col("accuracy") <= 500)
...
from pyspark.sql.functions import lag, when, sum, first, last, col, row_number
from pyspark.sql.window import Window
...
from pyspark.sql.functions import when, hour, minute, dayofweek, count, desc, first, col, sum, unix_timestamp
...
def classify_time_period(hour_col, minute_col, day_col):
    ...
    home_time_weekday = (total_minutes >= 1060) | (total_minutes < 530)  # 5:40 PM to 8:50 AM
    work_time_weekday = (total_minutes >= 580) & (total_minutes < 1020)  # 9:40 AM to 5:00 PM

    # Weekend pattern (Sunday)
    sunday = (day_col == 1)
    weekend_dwell = (total_minutes >= 660) & (total_minutes < 840)  # 11:00 AM to 2:00 PM
...
location_metrics = location_metrics.withColumn(
    ...
    .when(col("time_period") == "Work Time", col("total_dwell_time") * col("visit_frequency") * 1.1)
...
# Customize the plot
plt.title('Location Data Scatter Plot 6')
plt.xlabel('Longitude')
plt.ylabel('Latitude')

# Add a color bar to show different IMSIs
plt.colorbar(scatter, label='IMSI (encoded)')
...
clustered_locations_pd['cluster_label'] = clustered_locations_pd.apply(
    lambda row: 'Home' if row['time_period'] == 'Home Time' else
                ...
                'Weekend Dwell 1' if row['time_period'] == 'Weekend Time' else
                'Other',
    axis=1
)
...
# Save the map
map_path = "clustered_locations_visualization_6.html"
m.save(map_path)
