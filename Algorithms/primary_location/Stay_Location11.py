...
df = spark.read.csv("C:/Users/User/Downloads/11.csv", header=True, inferSchema=True)
...
# Define the expected time windows for home and work in seconds since midnight
home_time_start = 21 * 3600  # 9:00 PM in seconds
home_time_end = 10 * 3600    # 10:00 AM in seconds
work_time_start = 11 * 3600  # 11:00 AM in seconds
work_time_end = 20 * 3600    # 8:00 PM in seconds

# Calculate the start time in seconds since midnight
df_final = df_final.withColumn(
    "seconds_since_midnight",
    (F.hour("datetime") * 3600) + (F.minute("datetime") * 60) + F.second("datetime")
)

# Filter stays for potential home locations (expected home time)
home_stays = df_final.filter(
    (df_final.seconds_since_midnight >= home_time_start) |
    (df_final.seconds_since_midnight <= home_time_end)
)

# Filter stays for potential work locations (expected work time)
work_stays = df_final.filter(
    (df_final.seconds_since_midnight >= work_time_start) &
    (df_final.seconds_since_midnight <= work_time_end)
)

# Aggregate time spent at each location for potential home stays
home_agg = home_stays.groupBy("latitude", "longitude").agg(
    F.sum("time_at_location").alias("total_home_time")
)

# Aggregate time spent at each location for potential work stays
work_agg = work_stays.groupBy("latitude", "longitude").agg(
    F.sum("time_at_location").alias("total_work_time")
)

# Identify the most likely home location by the maximum time spent
most_likely_home = home_agg.orderBy(F.desc("total_home_time")).first()

# Check if a most likely home location was found
if most_likely_home:
    home_lat = most_likely_home["latitude"]
    home_lon = most_likely_home["longitude"]
else:
    home_lat = None
    home_lon = None

# Identify the most likely work location by the maximum time spent
most_likely_work = work_agg.orderBy(F.desc("total_work_time")).first()

# Check if a most likely work location was found
if most_likely_work:
    work_lat = most_likely_work["latitude"]
    work_lon = most_likely_work["longitude"]
else:
    work_lat = None
    work_lon = None

# Output the results
print(f"Most Likely Home Location: Latitude = {home_lat}, Longitude = {home_lon}")
print(f"Most Likely Work Location: Latitude = {work_lat}, Longitude = {work_lon}")
...
# Customize the plot
plt.title('Location Data Scatter Plot 11')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.legend()
...
# Save the map
map_path = "clustered_locations_visualization_11.html"
m.save(map_path)
